...
const { EXCHANGES, logger, todayFormat, mail, sendAlert } = require("../utils");

const methodExecution = async (payload, channel) => {
  try {
    await mail
      .init()
      .from(payload?.email_from ?? process.env.EMAIL_FROM)
      .to(payload?.email)
...
const initEmailServices = async () => {
  ...
  try {
    ...
    await channel.consume(
      ...
      async (msg) => {
        ...
        try {
          ...
          logger("email-services.txt", "email").write(
            `Success consume-email-${todayFormat("YYYY-MM-DD hh:mm:ss")}: ${JSON.stringify(parseData)}}\n`,
          );
        } catch (error) {
          console.info("error job", error);
          logger("email-services.txt", "email").write(
            `Failed consume-email-${todayFormat("YYYY-MM-DD hh:mm:ss")}: ${JSON.stringify(error)}}\n`,
          );
        }
        channel.ack(msg);
      },
    ...
    console.info(error);
    logger("email-services.txt", "email").write(
      `Error consume-email-${todayFormat("YYYY-MM-DD hh:mm:ss")}: ${error} - ${error.toString()}\n`,
    );
  }
};
